...
import { SubscriptionRequestDto } from './dto/create-subscription.dto';

@WebSocketGateway({ namespace: '/market', cors: { origin: '*' } })
@Injectable()
export class MsBinanceGateway
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  @WebSocketServer() server: Server;
  private readonly logger = new Logger(MsBinanceGateway.name);

  constructor(private readonly marketService: MsBinanceService) {}
  ...
  handleSubscribeOnce(client: Socket, payload: SubscriptionRequestDto) {
    ...
    this.logger.log(
      `Client ${client.id} requested one-time data for ${symbol}`,
    );
